...
router.get('/', async (req, res) => {
  try {
    ...
    res.json({ annonces, total, page: parseInt(page), limit: parseInt(limit) });
  } catch (err) {
...
router.get('/admin/all', auth, async (req, res) => {
  try {
    ...
    if (search) {
      where += ' AND (titre LIKE ? OR ville LIKE ? OR gouvernorat LIKE ?)';
      params.push(`%${search}%`, `%${search}%`, `%${search}%`);
    }
...
// GET /api/annonces/:id — détail par id (fallback)
